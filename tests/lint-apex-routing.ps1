...
.DESCRIPTION
    Parses twig-sdlc-v2-full.yaml and verifies:
...
$apexPath = Join-Path $repoRoot 'workflows' 'twig-sdlc-v2-full.yaml'
...
# ── Check 4: Sub-workflow agents have path fields ─────────────────────────
foreach ($name in $subWorkflowNames) {
    # Find agent block and check it has a path field
    if ($content -match "(?s)name:\s*$name\s*\n\s*type:\s*workflow.*?(?=\n\s*-\s*name:|\z)") {
        $agentBlock = $Matches[0]
        if ($agentBlock -notmatch 'path:\s*workflows/') {
            $violations += [PSCustomObject]@{
                Rule    = 'missing-workflow-path'
                Detail  = "Agent '$name' missing path to workflow YAML"
            }
        }
    }
}
