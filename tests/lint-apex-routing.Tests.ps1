BeforeAll {
    ...
    $script:ApexYaml = Join-Path $PSScriptRoot '..' 'workflows' 'twig-sdlc-v2-full.yaml'
}
...
Describe 'lint-apex-routing.ps1' {
    ...
    Context 'Production apex YAML validation' {
        ...
        It 'Passes on the real twig-sdlc-v2-full.yaml' {
...
agents:
  ...
  - name: planning
    type: workflow
    path: workflows/twig-sdlc-v2-planning.yaml
  - name: implementation
    type: workflow
    path: workflows/twig-sdlc-v2-implement.yaml
  - name: close_out
    type: workflow
    path: workflows/close-out.yaml
'@
            Set-Content (Join-Path $script:WorkflowsDir 'twig-sdlc-v2-full.yaml') $yaml
