...
.SYNOPSIS
    ...
    SDLC entry point. Drives the per-root worktree layout regardless of whether
...
.DESCRIPTION
    ...
    Two production bugs the per-root worktree contract eliminates by construction:
...
.PARAMETER Intent
    ...
    (`reset-root@polyphony`) instead of the entry-point workflow. The reset path skips
...
.EXAMPLE
    ...
    the polyphony entry-point workflow on work item 3085 inside it.
...
# Silently ignoring them on entry-workflow dispatches would mask operator
...
# per-root worktree contract operates against $commonDir regardless of
# whether $commonDir is `<repo>.git/` (bare) or `<repo>/.git/` (vanilla).

# ─── Reset intent diversion ──────────────────────────────────────────────────
#
# When -Intent reset, we are NOT launching the entry-point workflow and we want NONE of
...
# ignored, so an operator who accidentally passes entry-workflow knobs to
...
if ($Intent -eq 'reset') {
    ...
    # Pin a web port (consistent with entry-workflow launches; lets abort
    ...
    # Detached spawn — same transcript + exit-sidecar shape as the entry-point workflow.
...
Re-running the entry-point workflow against a completed work item produces a
