...
/// <summary>
/// Output envelope for <c>polyphony reset worktrees --root N</c> — removes
/// all linked worktrees under <c>{runs_root}/root-{N}/</c> via
/// <c>git worktree remove --force</c>, then deletes the root directory
/// tree.
///
...
public sealed record ResetWorktreesResult
{
    ...
    /// <summary>
    /// Resolved <c>{runs_root}/root-{N}/</c> path — what the verb scanned
    /// for worktrees. Empty when resolution failed.
    /// </summary>
    public string RootRunsRoot { get; init; } = string.Empty;
