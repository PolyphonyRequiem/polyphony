...
/// <summary>
/// Output of <c>polyphony pr open-impl-ado</c> — the Azure DevOps analogue of
/// <c>polyphony pr open-impl-pr</c>. Both verbs perform the same logical step
/// (open or reuse the PR promoting an <c>impl/{root}-{item}</c> branch into
/// its enclosing <c>mg/{root}_{mg_path}</c> branch). The platform identity
/// differs: ADO PRs live at <c>(organization, project, repository, prNumber)</c>,
/// not <c>(repoSlug, prNumber)</c>.
///
...
/// </summary>
public sealed record PrOpenImplAdoResult
{
    ...
    /// for cross-platform routing parity with <see cref="PrOpenImplResult"/>
    /// consumers (which echo a GitHub-style <c>owner/repo</c>). Empty when
    /// the verb errored before slug construction.
    /// </summary>
