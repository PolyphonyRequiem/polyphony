...
The polyphony SDLC engine drives multi-item, multi-batch concurrency: a single
root run fans out into per-item lifecycle dispatches (`plan-level`,
`actionable`, `implement-merge-group`, `feature-pr`) inside an
`root-batch-dispatch` loop with `max_concurrent: 3`. Several layers of state
...
~/projects/polyphony-runs/
  root-{root}/                           # plain container directory (NOT a worktree)
...
1. The main worktree is the **operator's workspace** and is never targeted
   ...
   on Windows).
2. The root run gets its own per-root tree under `polyphony-runs/`. The
   root container is a **plain directory**, not a worktree (avoiding the
   "child worktrees show up as untracked content of the parent worktree"
   conflict).
3. Per-item worktrees are **siblings** under the root container. Naming
...
4. Path resolution lives in **one place**: `polyphony worktree
   {init-root,create}` derives `runs_root` from `git --git-common-dir` and
   constructs `{runs_root}/root-{root}/{slug}/`. PowerShell never mirrors
   the resolver.
5. The launcher reads `runs_root` and `main_worktree_path` from
   `polyphony worktree init-root`'s output envelope (PR 3). Defense-in-depth:
   refuses if init-root's derived path is or is inside main.
...
| Filesystem  | Sibling per-item dirty state racing on shared HEAD   | **Per-item sibling worktrees** under root container  |
...
`for_each` serializes sibling spawns within a batch; `polyphony worktree
...
per-root-worktree advisory lock (e.g. `flock` on a marker file in the root
...
- **Hijack** structurally impossible: launcher has no path that targets
  the main worktree (verified by Pester refusal tests + PR 3 init-root
  PathBoundary check).
- **Cross-contamination** structurally impossible: each root run lives in
