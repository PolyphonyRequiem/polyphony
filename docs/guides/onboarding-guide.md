...
- The SDLC launcher (`scripts/Invoke-PolyphonySdlc.ps1`) refuses to dispatch into the operator's main worktree; per-root-run path derivation makes the hijack bug structurally impossible regardless of whether the underlying source repo is bare or vanilla.
...
Once validation passes, you're ready to drive a root through the polyphony
SDLC pipeline.
...
root work item end-to-end. The only required input is `root_id`;
...
[`docs/decisions/polyphony-entry-workflow.md`](decisions/polyphony-entry-workflow.md) for the full
...
The polyphony.yaml re-derives the right leg per item per batch from observable
