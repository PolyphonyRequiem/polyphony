...
| Term | Definition |
|---|---|
| **Polyphony tag** | The `polyphony:*` namespace stamped on every work item the polyphony pipeline owns. The bare `polyphony` tag marks an in-scope descendant; `polyphony:root` marks a root; `polyphony:planned` is a status sub-tag set by the planner. Authoritative spec: `docs/polyphony-tags.md`. Typed encoding: the `PolyphonyTag` DU in `src/Polyphony/Models/PolyphonyTag.cs` (state / intent / facet / scope variants). |
...
| <a id="polyphony-reset"></a>**Reset (polyphony reset)** | The `polyphony reset --root-id N` verb that scrubs all polyphony-authored state for a single root: ADO tags matching the `PolyphonyTag` DU, polyphony-authored comments (archived to sidecar first), per-root state dir, per-root branches (local + remote), and per-run worktrees. The primary remediation path when the launcher's terminal-state pre-flight refusal fires. Authoritative spec: `docs/polyphony-reset.md`. |
| <a id="polyphony-tag-du"></a>**PolyphonyTag DU** | The typed discriminated union (`src/Polyphony/Models/PolyphonyTag.cs`) that is the single source of truth for which tags polyphony owns. Four variants: State (`polyphony:planned`), Intent (`polyphony:root`), Facet (`polyphony:facets=<value>`), Scope (bare `polyphony`). Consumed by the `reset` verb, `scope` verbs, and tag-stripping logic. |
...
| **PR kind** | One of `impl_pr`, `merge_group_pr`, `plan_pr`, `evidence_pr`, `feature_pr`. The unit on which review/merge policy is configured. | — |
| **Merge-group naming asymmetry** | The token `mg` is reserved for **branch grammar only** — branch names like `mg/{r}_{mg_path}`, the CLI flag `--mg-path`, and `pr open-mg-pr` / `branch ensure-mg` sub-verbs that operate directly on those branches. **Everywhere else** — config keys (`merge_group_pr`, `merge_group_branch`, `concurrency.max_concurrent_*`), CLI verb groups (`polyphony merge-group`), C# types (`MergeGroupCommands`, `MergeGroupNestingDecisionResult`), workflow filenames (`implement-merge-group.yaml`), and prose — uses the long form. The split is intentional: branch tokens stay terse for `git log` / `git switch` ergonomics; everything else stays self-describing. The G2 retirement (Polyphony 2.4.0) collapsed the legacy `pg_*` aliases entirely; `mg_*` config keys fail loudly via raw-YAML pre-validation. |
