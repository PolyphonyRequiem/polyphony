...
- 📌 2026-05-29: **Polyphony repo merge convention**: Dominant pattern is **squash** for single-concept PRs (recent examples: #520, #519, #518 all show squash-style single-line merge titles with PR numbers in parentheses). Secondary pattern is **merge commit** for multi-author or multi-phase history (example: PR #517 uses "Merge pull request" header). Applied this convention: PR #546 (single ADR file) → squash; PR #547 (Wagner's refactor + Liszt's bug fixes) → merge commit. Both merged cleanly to main (eab39cb). Worktree constraints prevented local branch cleanup but remote branches deleted successfully.
