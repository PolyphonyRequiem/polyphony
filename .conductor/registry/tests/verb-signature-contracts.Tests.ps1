<#
.SYNOPSIS
    Verb-signature contract tests for the polyphony pipeline.

.DESCRIPTION
    Pins the EXACT polyphony CLI invocations the polyphony pipeline
    ...
    that took down the dogfood run against ADO Epic #3043:
      ...
      • Helper script (batch-integrator.ps1) keeps the old
...
BeforeAll {
    ...
    $script:RootYaml = ConvertFrom-Yaml (Get-Content (Join-Path $script:WorkflowsDir 'polyphony.yaml') -Raw)
...
Describe 'polyphony.yaml :: preflight_ensure_branch invokes branch ensure-feature with --branch' {
    It 'uses the polyphony command' {
        (Get-AgentCommand $script:RootYaml 'preflight_ensure_branch') | Should -Be 'polyphony'
    }
    It 'first arg is "branch"' {
        (Get-AgentArgs $script:RootYaml 'preflight_ensure_branch')[0] | Should -Be 'branch'
    }
    It 'second arg is "ensure-feature"' {
        (Get-AgentArgs $script:RootYaml 'preflight_ensure_branch')[1] | Should -Be 'ensure-feature'
    }
    It 'passes branch via --branch flag (NOT positional)' {
        $args = Get-AgentArgs $script:RootYaml 'preflight_ensure_branch'
        $args | Should -Contain '--branch'
        # The branch value should immediately follow the --branch flag.
        # Per branch-model spec: feature/{root_id} (no root- prefix; the
        # root- sub-prefix was a YAML-side drift fixed in PR #176).
        $idx = [Array]::IndexOf($args, '--branch')
        $args[$idx + 1] | Should -Match '^feature/\{\{ workflow\.input\.root_id \}\}$'
    }
...
Describe 'polyphony.yaml :: init_manifest agent exists and runs before build_worklist' {
    It 'init_manifest agent is present' {
        $agent = $script:RootYaml.agents | Where-Object { $_.name -eq 'init_manifest' }
        $agent | Should -Not -BeNullOrEmpty
    }
    It 'init_manifest invokes pwsh -File manifest-bootstrap.ps1' {
        $args = Get-AgentArgs $script:RootYaml 'init_manifest'
        (Get-AgentCommand $script:RootYaml 'init_manifest') | Should -Be 'pwsh'
        ($args -join ' ') | Should -Match 'manifest-bootstrap\.ps1'
    }
    It 'init_manifest threads workflow inputs (root_id, organization, project)' {
        $args = Get-AgentArgs $script:RootYaml 'init_manifest'
        $argsText = $args -join ' '
        $argsText | Should -Match '\-RootId'
    ...
    It 'preflight_ensure_branch routes to init_manifest on success (init_manifest is reachable)' {
        $agent = $script:RootYaml.agents | Where-Object { $_.name -eq 'preflight_ensure_branch' }
...
Describe 'polyphony.yaml :: declare_root agent stamps polyphony:root before build_worklist' {
    ...
    It 'declare_root agent is present' {
        $agent = $script:RootYaml.agents | Where-Object { $_.name -eq 'declare_root' }
        $agent | Should -Not -BeNullOrEmpty
    }
    It 'declare_root invokes polyphony root declare with --work-item' {
        (Get-AgentCommand $script:RootYaml 'declare_root') | Should -Be 'polyphony'
        $args = Get-AgentArgs $script:RootYaml 'declare_root'
        $argsText = $args -join ' '
        $argsText | Should -Match '^root declare'
        $argsText | Should -Match '\-\-work-item'
    }
    It 'declare_root threads root_id from workflow input' {
        $args = Get-AgentArgs $script:RootYaml 'declare_root'
        ($args -join ' ') | Should -Match 'workflow\.input\.root_id'
    }
    It 'declare_root routes to outer_loop_init on success (PR #9 inserted outer_loop_init between declare_root and build_worklist; outer_loop_init then routes to build_worklist)' {
        ...
        # holds transitively.
        $initAgent = $script:RootYaml.agents | Where-Object { $_.name -eq 'outer_loop_init' }
...
Describe 'polyphony.yaml :: build_worklist invokes worklist build with --root-id' {
    It 'first three args are "worklist","build","--root-id"' {
        $args = Get-AgentArgs $script:RootYaml 'build_worklist'
        $args[0] | Should -Be 'worklist'
        $args[1] | Should -Be 'build'
        $args[2] | Should -Be '--root-id'
    }
    It 'passes --json flag' {
        (Get-AgentArgs $script:RootYaml 'build_worklist') | Should -Contain '--json'
    }
    It 'does not pass root_id positionally (regression — pre-fix bug)' {
        $args = Get-AgentArgs $script:RootYaml 'build_worklist'
        # Pre-fix shape was: ["worklist","build","{{ root_id }}","--json"] (no --root-id).
        if ($args[2] -notmatch '^--' -and $args -notcontains '--root-id') {
            throw "build_worklist is passing root_id positionally — verb requires --root-id flag"
        }
    }
}

Describe 'polyphony.yaml :: check_conflicts invokes edges check positionally (post PR #158)' {
    It 'first two args are "edges","check"' {
        $args = Get-AgentArgs $script:RootYaml 'check_conflicts'
        $args[0] | Should -Be 'edges'
        $args[1] | Should -Be 'check'
    }
    It 'passes root_id positionally (NOT via --work-item)' {
        $args = Get-AgentArgs $script:RootYaml 'check_conflicts'
        $args | Should -Not -Contain '--work-item'
        # The third arg should be the root_id template substitution, not a flag.
        $args[2] | Should -Not -Match '^--'
        $args[2] | Should -Match 'root_id'
    }
...
Describe 'polyphony.yaml :: check_conflicts has explicit error route to worklist_failure_gate' {
    It 'routes envelope errors before evaluating has_conflicts' {
        $agent = $script:RootYaml.agents | Where-Object { $_.name -eq 'check_conflicts' }
...
Describe 'batch-integrator.ps1 :: edges check uses positional root_id (post PR #158)' {
    BeforeAll {
        $script:BatchIntegrator = Join-Path $script:ScriptsDir 'batch-integrator.ps1'
        $script:BatchIntegratorRaw = Get-Content $script:BatchIntegrator -Raw
    }
    It 'invokes edges check positionally — no --work-item flag' {
        # Look for the actual invocation line, not docstring references.
        $invocationLines = $script:BatchIntegratorRaw -split "`n" | Where-Object {
            $_ -match '\$PolyphonyExe\s+edges\s+check'
        }
        $invocationLines | Should -Not -BeNullOrEmpty -Because 'batch-integrator must invoke edges check'
        foreach ($line in $invocationLines) {
            $line | Should -Not -Match '--work-item' -Because "batch-integrator drifted with PR #158: $line"
        }
    }
    It 'docstring references positional form' {
        # The header docstring should describe the current invocation form.
        $script:BatchIntegratorRaw | Should -Not -Match 'edges check --work-item' -Because 'docstring references stale --work-item form'
    }
}
