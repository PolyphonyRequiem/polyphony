BeforeAll {
    $script:LintScript = Join-Path $PSScriptRoot 'lint-polyphony.ps1'
    $script:WorkflowsDir = Join-Path $PSScriptRoot '..' 'workflows'
    $script:ScriptsDir = Join-Path $PSScriptRoot '..' 'scripts'

    $script:RootYaml = Join-Path $script:WorkflowsDir 'polyphony.yaml'
    $script:BatchYaml = Join-Path $script:WorkflowsDir 'root-batch-dispatch.yaml'
    $script:ItemYaml = Join-Path $script:WorkflowsDir 'root-item-dispatch.yaml'
    ...
    $script:BatchIntegratorScript = Join-Path $script:ScriptsDir 'batch-integrator.ps1'
}

Describe 'lint-polyphony.ps1' {

    Context 'Production polyphony.yaml validation' {

        It 'Passes on the real polyphony.yaml + sub-workflows' {
            $script:RootYaml | Should -Exist
            $script:BatchYaml | Should -Exist
            $script:ItemYaml | Should -Exist
            $output = pwsh -NoProfile -File $script:LintScript 2>&1
            $LASTEXITCODE | Should -Be 0
        }

        It 'Real polyphony.yaml passes the strict-undefined lint' {
        ...
        It 'batch-integrator.ps1 is present and parses' {
            $script:BatchIntegratorScript | Should -Exist
            $tokens = $null
            $errors = $null
            [System.Management.Automation.Language.Parser]::ParseFile(
                $script:BatchIntegratorScript, [ref]$tokens, [ref]$errors) | Out-Null
            $errors | Should -BeNullOrEmpty
        }
    }
...
workflow:
  name: root-batch-dispatch
  entry_point: integrate_batch
  metadata:
    min_polyphony_version: "1.0.1"
  limits:
    max_iterations: 50
...
output:
  ok: "{% if integrate_batch is defined %}true{% else %}false{% endif %}"
  flag: "{{ x | string | lower }}"

agents:
  - name: integrate_batch
    type: script
    command: pwsh
    args:
      - "-NoProfile"
...
'@
            ...
            $script:TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("root-lint-" + [Guid]::NewGuid().ToString('N'))
            ...
            Copy-Item $script:LintScript -Destination (Join-Path $script:TempTests 'lint-polyphony.ps1') -Force
            # Drop the strict-undefined lint too (the root lint may delegate)
            $strict = Join-Path $PSScriptRoot 'lint-strict-undefined.ps1'
            if (Test-Path $strict) {
            ...
            'param() exit 0' | Out-File -FilePath (Join-Path $script:TempScripts 'batch-integrator.ps1') -Encoding utf8
        }
        ...
        BeforeEach {
            ...
            $script:ValidApexYaml | Out-File -FilePath (Join-Path $script:TempWorkflows 'polyphony.yaml') -Encoding utf8
            $script:ValidWaveYaml | Out-File -FilePath (Join-Path $script:TempWorkflows 'root-batch-dispatch.yaml') -Encoding utf8
            $script:ValidItemYaml | Out-File -FilePath (Join-Path $script:TempWorkflows 'root-item-dispatch.yaml') -Encoding utf8
        }

        It 'Passes on a synthetic baseline trio of YAMLs' {
            $lint = Join-Path $script:TempTests 'lint-polyphony.ps1'
            $output = pwsh -NoProfile -File $lint 2>&1
            $LASTEXITCODE | Should -Be 0
        }

        It 'Fails when polyphony.yaml has the wrong workflow name' {
            $bad = $script:ValidApexYaml -replace 'name:\s*polyphony', 'name: not-polyphony'
            $bad | Out-File -FilePath (Join-Path $script:TempWorkflows 'polyphony.yaml') -Encoding utf8
            $lint = Join-Path $script:TempTests 'lint-polyphony.ps1'
            $output = pwsh -NoProfile -File $lint 2>&1
        ...
        It 'Fails when a hardcoded process-template type name is present (P5)' {
            ...
            $bad = $script:ValidApexYaml -replace 'root_id: "\{\{ workflow\.input\.root_id \}\}"', 'root_id_label: "Epic root"'
            $bad | Out-File -FilePath (Join-Path $script:TempWorkflows 'polyphony.yaml') -Encoding utf8
            $lint = Join-Path $script:TempTests 'lint-polyphony.ps1'
            $output = pwsh -NoProfile -File $lint 2>&1
            $LASTEXITCODE | Should -Be 1
        ...
        It 'Fails when a sub-workflow output map omits is-defined guards (M3)' {
            ...
            $bad | Out-File -FilePath (Join-Path $script:TempWorkflows 'root-batch-dispatch.yaml') -Encoding utf8
            $lint = Join-Path $script:TempTests 'lint-polyphony.ps1'
            $output = pwsh -NoProfile -File $lint 2>&1
            $LASTEXITCODE | Should -Be 1
        ...
        It "Lint Check 9 fires when a script's post-state 'twig sync' is removed" {
            ...
            # polyphony.yaml: calls `twig state` but no follow-up sync.
...
"@
  ...
  - name: notes_only
    ...
    routes:
      - to: `$end
"@
            $ok | Out-File -FilePath (Join-Path $script:TempWorkflows 'polyphony.yaml') -Encoding utf8
            $lint = Join-Path $script:TempTests 'lint-polyphony.ps1'
            $output = pwsh -NoProfile -File $lint 2>&1
            $LASTEXITCODE | Should -Be 0
        }
        ...
        It 'Fails when the dispatch shape lacks a named lifecycle branch (Phase 7 follow-up)' {
            $stripped = $script:ValidItemYaml -replace 'lifecycle_dispatch_placeholder', 'plan_level'
            $stripped = $stripped + @"
  ...
  - name: actionable
    ...
    routes:
      - to: `$end
  - name: implement_merge_group
    type: workflow
    workflow: ./implement-merge-group.yaml
    routes:
      - to: `$end
"@
            $stripped | Out-File -FilePath (Join-Path $script:TempWorkflows 'root-item-dispatch.yaml') -Encoding utf8
            $lint = Join-Path $script:TempTests 'lint-polyphony.ps1'
            $output = pwsh -NoProfile -File $lint 2>&1
            $LASTEXITCODE | Should -Be 1
        ...
        It 'Fails when a lifecycle workflow file is not referenced (Phase 7 follow-up)' {
            $renamed = $script:ValidItemYaml -replace 'lifecycle_dispatch_placeholder', 'plan_level'
            $renamed = $renamed + @"
  ...
  - name: feature_pr
    ...
    routes:
      - to: `$end
"@
            $renamed | Out-File -FilePath (Join-Path $script:TempWorkflows 'root-item-dispatch.yaml') -Encoding utf8
            $lint = Join-Path $script:TempTests 'lint-polyphony.ps1'
            $output = pwsh -NoProfile -File $lint 2>&1
            $LASTEXITCODE | Should -Be 1
...
agents:
  ...
  - name: feature_pr
    ...
    routes:
      - to: $end
'@
            $bare | Out-File -FilePath (Join-Path $script:TempWorkflows 'root-item-dispatch.yaml') -Encoding utf8
            $lint = Join-Path $script:TempTests 'lint-polyphony.ps1'
            $output = pwsh -NoProfile -File $lint 2>&1
            $LASTEXITCODE | Should -Be 1
    ...
    Context 'Per-item lifecycle dispatch wiring (Phase 7 follow-up — real YAMLs)' {
        ...
        It 'Real root-item-dispatch.yaml has all four lifecycle dispatch nodes by name' {
        ...
        It 'Real root-item-dispatch.yaml references all four lifecycle workflow files' {
            $itemContent = Get-Content $script:ItemYaml -Raw
            foreach ($r in @('./plan-level.yaml', './actionable.yaml', './implement-merge-group.yaml', './feature-pr.yaml')) {
                $itemContent.Contains($r) | Should -BeTrue -Because "Expected '$r' to be referenced from root-item-dispatch.yaml"
            }
        }

        It 'Real root-item-dispatch.yaml output map bubbles up plan_level renegotiation_pending' {
            $itemContent = Get-Content $script:ItemYaml -Raw
            $itemContent | Should -Match 'plan_level\.output\.renegotiation_pending'
        }

        It 'Real root-batch-dispatch.yaml output map declares batch-aggregated renegotiation_pending' {
            $batchContent = Get-Content $script:BatchYaml -Raw
            $batchContent | Should -Match '(?m)^\s+renegotiation_pending:\s*'
        }

        It 'Real polyphony.yaml output map declares root-level renegotiation_pending rollup' {
            $rootContent = Get-Content $script:RootYaml -Raw
            $rootContent | Should -Match '(?m)^\s+renegotiation_pending:\s*'
        }

        It 'Real polyphony.yaml has a renegotiation_gate human gate and renegotiation_summary script' {
            $rootContent = Get-Content $script:RootYaml -Raw
            $rootContent | Should -Match '(?m)^\s*-\s+name:\s*renegotiation_gate\s*$'
            $rootContent | Should -Match '(?m)^\s*-\s+name:\s*renegotiation_summary\s*$'
        }

        It 'Real root-item-dispatch.yaml routes from spawn_worktree to all four lifecycle nodes' {
            $itemContent = Get-Content $script:ItemYaml -Raw
            foreach ($n in @('plan_level', 'actionable', 'implement_merge_group', 'feature_pr')) {
    ...
    Context 'State-mutation durability — AB#3129 (sister of AB#3126)' {
        ...
        It "Real polyphony.yaml > close_mark_satisfied has fail-fast prologue + post-state sync" {
            $rootContent = Get-Content $script:RootYaml -Raw
            $pattern = '(?ms)^  - name: close_mark_satisfied\b.*?(?=^  - name: |\Z)'
            $match = [regex]::Match($rootContent, $pattern)
        ...
        It "Real root-item-dispatch.yaml > satisfied has fail-fast prologue + post-state sync" {
            $itemContent = Get-Content $script:ItemYaml -Raw
