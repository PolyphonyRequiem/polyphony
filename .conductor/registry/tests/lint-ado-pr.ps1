...
if ($content -notmatch 'name:\s*cap_auto_fail\b') {
    ...
    if ($terminalMatch.Success -and $terminalMatch.Value -notmatch '"cap-auto-fail"') {
        ...
        # shared post-pr-warning-comment.ps1 helper so platform-specific
        # comment wiring lives in exactly one place.
        if ($content -notmatch 'name:\s*pr_warning_stamp\b') {
            ...
            # Platform-arg sanity: ado-pr.yaml must pass -Platform ado, not github.
            if ($stampBlock -notmatch '(?m)^\s*-\s*"ado"') {
                $violations += [PSCustomObject]@{
                    Rule   = 'pr-warning-stamp-wrong-platform'
                    Detail = "AB#3217: 'pr_warning_stamp' in ado-pr.yaml must pass '-Platform ado' to post-pr-warning-comment.ps1 (not 'github')."
                }
            }
        }
